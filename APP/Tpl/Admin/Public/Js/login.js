...
function change_code(obj){
	$("#code").attr("src",vertifyURL+'/'+Math.random());
	return false;
}
//登录验证  1为空   2为错误
//var validate={username:1,password:1,code:1};
var validate={username:0,password:0,code:1};
$(function(){
...
$(function(){
	...
	$("input[name='code']").blur(function(){
		...
		$.post(CONTROL+"/checkcode",{code:code.val().trim()},function(data){
			if(data.codestat==true){
				validate.code=0;
				code.parent().find("span").remove();
				
				console.log('可以尝试后台验证');
