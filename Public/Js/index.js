...
$(function () {
	...
	$( '.close' ).click( function (e) {
		...
		var delPostHandleUrl = $(this).attr('href');
		$( this ).parents( 'dl' ).fadeOut('slow');
		
		//增删改操作由页面跳转，调整为Ajax后台运行
		$.get(delPostHandleUrl,
				function(data){
			if(data.del_status){
				alert("删除成功");
			}
	...
	$('#send-btn').click(function(){
		...
		$.post(addPostHandleUrl,{username:username.val(),content:content.val()},
				function(data){
			if(data.status){
				var str = '<dl class="paper a1">' +
						'<dt><span class="username">'+ data.username + '</span>' +
						'<span class="num">'+data.id+'</span>' +
						'</dt>' +
						'<dd class="content">'+data.content+'</dd>'+
						'<dd class="bottom">'+
                        '<span class="time">'+data.time+'</span>'+
                        '<a href="/php_demo/shizhan/10/10wish/index.php/Index/del/id/'+data.id+'.html" class="close"></a>'+
                '</dd></dl>';
